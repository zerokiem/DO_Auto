...
DOWNLOAD_DIR = Path(r"D:\OneDrive - NPT\9. Jobs\Van_ban\VB_Chu_tri_da_XL")
DOWNLOAD_DIR.mkdir(parents=True, exist_ok=True)

LOG_FILE = DOWNLOAD_DIR / "Tong_hop_VB_chu_tri_da_XL.xlsx"
...
def init_excel_log() -> None:
    if LOG_FILE.exists():
        ...
        ws = wb["Tong_hop"] if "Tong_hop" in wb.sheetnames else wb.active
        if ws.title != "Tong_hop":
            ws.title = "Tong_hop"
...
def save_debug(page, name: str) -> None:
    path = f"debug_chu_tri_da_xl_{name}.png"
    try:
...
def open_chu_tri_da_xu_ly_list(page) -> bool:
    print("\n--- Vào Văn bản đến / Đã xử lý / Chủ trì ---")
    ...
    try:
        ...
        try:
            ...
            print("✅ Click sidebar Văn bản bằng locator div filter.")
        except Exception as e2:
            print("❌ Không click được sidebar Văn bản:", e2)
            save_debug(page, "sidebar_van_ban_failed")
            return False

    wait(page, 1000)

    if not safe_click(page.get_by_role("link", name=re.compile(r"Đã xử lý", re.I)), "Link Đã xử lý", timeout=10000):
        save_debug(page, "link_da_xu_ly_failed")
        return False

    wait(page, 2000)

    # Tab Chủ trì thường là tab mặc định, nhưng vẫn click lại cho chắc.
    try:
        page.get_by_role("tab", name=re.compile(r"Chủ trì", re.I)).click(timeout=10000)
        print("✅ Click tab Chủ trì.")
    except Exception as e:
        print("⚠️ Không click được tab Chủ trì bằng role, thử bằng text:", e)
        try:
            page.get_by_text(re.compile(r"Chủ trì\s*\(\d+\)", re.I)).click(timeout=10000)
            print("✅ Click tab Chủ trì bằng text.")
        except Exception as e2:
            # Nếu đang ở tab Chủ trì sẵn thì vẫn có thể tiếp tục nếu thấy row.
            print("⚠️ Không click được tab Chủ trì, kiểm tra xem danh sách có sẵn không:", e2)

    try:
        page.locator("tr.mat-row").first.wait_for(state="visible", timeout=12000)
        print("✅ Đã thấy danh sách văn bản Chủ trì - Đã xử lý.")
        wait(page, 1000)
        return True
    except Exception as e:
        print("❌ Không thấy danh sách văn bản:", e)
        save_debug(page, "chu_tri_list_not_found")
        return False
...
def extract_document_info_from_row(row) -> Dict[str, str]:
    ...
    # Số VB: div.vb-item > div:nth-child(1) > span:nth-child(1)
    try:
        data["so_vb"] = clean_text(vb_item.locator("div").nth(0).locator("span").nth(0).inner_text(timeout=3000))
    except Exception:
        pass

    # Ngày VB
    try:
        raw_ngay_vb = vb_item.locator("div").nth(0).locator("span").nth(1).inner_text(timeout=3000)
        data["ngay_vb"] = clean_date_vb(raw_ngay_vb)
    except Exception:
        pass

    # Nơi phát hành / Nơi gửi
    try:
        data["noi_phat_hanh"] = clean_text(vb_item.locator("div").nth(1).locator("span.text-blue-600").first.inner_text(timeout=3000))
    except Exception:
        try:
            data["noi_phat_hanh"] = clean_text(vb_item.locator("span.text-blue-600").first.inner_text(timeout=3000))
        except Exception:
            pass

    # Trích yếu
    try:
        data["trich_yeu"] = clean_text(vb_item.locator("> span").first.inner_text(timeout=3000))
    except Exception:
        try:
            data["trich_yeu"] = clean_text(vb_item.locator("span.unread").first.inner_text(timeout=3000))
        except Exception:
            pass

    # Khối chỉ đạo: section.text-blue-600 section
    try:
        chi_dao_text = vb_item.locator("section.text-blue-600 section").first.inner_text(timeout=3000)
        parsed = parse_chi_dao_block(chi_dao_text)
        data.update(parsed)
    except Exception:
        # Có vài VB đã xử lý nhưng không có block chỉ đạo hiển thị; để trống, vẫn ghi được các trường còn lại.
        pass
...
def click_document_row(row) -> bool:
    ...
    # Với Đã xử lý - Chủ trì, click icon flag trong dòng thường mở văn bản ổn định.
    click_candidates = [
...
def wait_pdf_ready(page) -> bool:
    ...
    try:
        page.locator("#download").wait_for(state="visible", timeout=5000)
        wait(page, 800)
...
def download_current_document(page, data: Dict[str, str], stt: int) -> Optional[Path]:
    ...
    for attempt in range(1, 4):
        print(f"Thử tải lần {attempt}/3...")
        try:
            with page.expect_download(timeout=30000) as download_info:
                try:
                    page.get_by_role("button", name=re.compile(r"Tải xuống|Download", re.I)).click(timeout=5000)
                    print("✅ Click nút Tải xuống bằng role.")
                except Exception:
                    print("⚠️ Role Tải xuống không click được, thử #download.")
                    page.locator("#download").click(timeout=5000)
            ...
            print("⚠️ Không bắt được download event ở lần này.")
            wait(page, 2000)
        except Exception as e:
            print("⚠️ Lỗi tải file:", e)
            wait(page, 2000)

    print("❌ Tải file thất bại sau 3 lần.")
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        try:
            if not open_chu_tri_da_xu_ly_list(page):
                print("Không vào được danh sách Chủ trì - Đã xử lý.")
                return
            ...
            while processed < MAX_DOCUMENTS:
                print("\n" + "=" * 70)
                print(f"XỬ LÝ VĂN BẢN THỨ {current_index + 1} TRONG DANH SÁCH | ĐÃ GHI {processed}/{MAX_DOCUMENTS}")
                print("=" * 70)
                ...
                try:
                    row = get_document_row(page, current_index)
                except Exception as e:
